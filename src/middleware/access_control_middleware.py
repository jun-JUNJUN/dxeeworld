...
class AccessControlMiddleware:
    ...
    async def process_request(
        ...
        try:
            ...
            if (self.last_config_load is None or
                (datetime.now(timezone.utc) - self.last_config_load).seconds > self.reload_interval):
    ...
    async def validate_configuration(self, rules_config: str) -> Result[bool, AccessControlError]:
        ...
        try:
            ...
            for entry in rule_entries:
                ...
                if not pattern or not permissions:
                    return Result.failure(AccessControlError(f"Empty pattern or permissions: {entry}"))
