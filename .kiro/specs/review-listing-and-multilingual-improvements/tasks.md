...
- [ ] 12. 残りの実装タスク
- [ ] 12.1 レビュー一覧ページの修正
  - **Issue:** 現在、`/review`ページで1社しか表示されない問題を修正
  - `templates/reviews/list.html`のテンプレート構文エラーを全て修正（Tornado構文に準拠）
  - `CompanySearchService.search_companies()`が正しくすべての企業を返すことを確認
  - デバッグログを追加して、返される企業数を確認
  - ブラウザキャッシュクリアの手順をドキュメント化
  - _Requirements: 1.3, 1.9, 1.10_

- [ ] 12.2 レビュー集計の自動実行
  - **Issue:** レビュー投稿時に`ReviewAggregationService`が自動的に呼ばれていない
  - `ReviewCreateHandler`のレビュー保存成功時に集計サービスを呼び出す
  - リアルタイム集計の実装（レビュー投稿直後に`Company.review_summary`を更新）
  - エラーハンドリング: 集計失敗時もレビュー投稿は成功させる（Graceful Degradation）
  - 集計実行ログの追加
  - _Requirements: 1.3, 1.8_

- [ ] 12.3 既存レビューの集計確認と修正
  - 既存の7件のレビューがすべて正しく集計されているか確認
  - `migrate_initial_aggregation.py`を再実行して、すべての企業の集計を更新
  - バリデーションスクリプトで集計の正確性を検証
  - _Requirements: 1.3_

- [ ] 13. E2E/UIテスト実装
- [ ] 13.1 レビュー投稿ユーザージャーニーテスト（日本語）
...
- [ ] 13.4 バリデーションエラー表示テスト
  - バリデーションエラーメッセージの表示位置と色を検証するテストを実装
  - 複数エラーの同時表示を検証するテストを実装
  - _Requirements: 7.5_

- [ ] 13.5 レビュー一覧アクセス制御UIテスト
  - 未認証ユーザーのプレビュー表示（伏せ字）を検証するテストを実装
  - 認証済みレビュー投稿者の全企業表示を検証するテストを実装
  - 認証済み非投稿者の制限メッセージ表示を検証するテストを実装
  - _Requirements: 1.1, 1.3, 1.7_

- [ ] 13.6 レビュー一覧ページネーションテスト
  - 20社以上の企業がある場合のページネーション動作を検証するテストを実装
  - ページ切り替え時の正しいデータ取得を検証するテストを実装
  - _Requirements: 1.9_
