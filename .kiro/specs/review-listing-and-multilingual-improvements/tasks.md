...
- [ ] 1. データモデル拡張とマイグレーション準備
- [ ] 1.1 Review モデルの多言語フィールド追加
...
- [ ] 2. アクセス制御層の実装
- [ ] 2.1 UserService にレビュー投稿履歴管理機能を追加
  - 最終レビュー投稿日時を取得する機能を実装
  - 最終レビュー投稿日時を更新する機能を実装
  - 1年以内のレビュー投稿履歴をチェックする機能を実装
  - _Requirements: 1.7, 1.8_

- [ ] 2.2 AccessControlMiddleware にレビュー一覧アクセス制御を追加
  - ユーザーの認証状態を判定する機能を拡張
...
- [ ] 4. 雇用期間自動入力とバリデーションの実装
- [ ] 4.1 雇用状態選択時の自動入力JavaScript実装
...
- [ ] 6. レビュー投稿確認画面の実装
- [ ] 6.1 ReviewCreateHandler に確認画面モードを追加
...
- [ ] 6.2 確認画面テンプレートの作成
  ...
  - 他の2言語の翻訳コメントを表示する機能を実装（元言語が英語以外の場合）
  - 「投稿」ボタンと「戻る」ボタンを実装
  - _Requirements: 3.2, 3.3, 3.4, 3.5, 3.6_

- [ ] 6.3 確認画面のナビゲーション機能実装
  - 「戻る」ボタンクリック時に入力内容を保持してフォーム画面に戻る機能を実装
  - 「投稿」ボタンクリック時にレビュー保存処理を実行する機能を実装
  - フォームデータの一時保存機能（セッションまたは隠しフィールド）を実装
  - _Requirements: 3.7, 3.8_

- [ ] 7. レビュー保存処理の多言語対応
- [ ] 7.1 ReviewSubmissionService の多言語データ保存機能拡張
...
- [ ] 7.2 レビュー投稿時の User.last_review_posted_at 更新
  - レビュー保存成功時に UserService を呼び出して最終投稿日時を更新する機能を実装
  - 更新処理の例外ハンドリングを実装
  - _Requirements: 1.8_

- [ ] 8. レビュー投稿完了フィードバックの実装
- [ ] 8.1 投稿成功メッセージの表示機能実装
