...
#### 11.8.1 データソースフィールドの非表示

**要件**:
- 企業詳細ページで「データソース」フィールドをUI上に表示しない
- データベースには引き続きデータソース情報を保持する
- バックエンドでのデータ管理・追跡目的のみで使用

**理由**:
- エンドユーザーにとって「データソース」情報は必要ない
- UI の簡素化とユーザーエクスペリエンスの向上
- 内部管理情報はバックエンドで保持

**実装方法**:
- テンプレート (`templates/companies/detail.html`) でデータソースフィールドの表示を削除
- データベース構造とモデル定義は変更しない
- CSV インポート機能など、バックエンド処理は影響を受けない

### 11.9 ドキュメンテーション要件

**目的**:
- 実装後の保守性向上のため、ページ構成と表示項目を文書化する
- 新規開発者がコードベースを理解しやすくする
- 仕様変更時の影響範囲を把握しやすくする

**要件**:
- 各ページの構成を入れ子構造（ツリー形式）で記載すること
- 表示項目の階層関係を明確にすること
- requirements.md と design.md の両方に記載すること

**詳細なページ構成は requirements.md の「ページ構成と表示項目」セクションを参照**
