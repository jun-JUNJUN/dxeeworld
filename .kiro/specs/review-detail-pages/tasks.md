...
- [x] 9.3 ナビゲーション統合のテスト
  ...
  - ✅ テスト実装と検証完了 (tests/test_navigation_integration.py)
  - _Requirements: 11.7.1, 11.7.2_

- [x] 10. UI表示制約の実装
- [x] 10.1 企業詳細ページからデータソースフィールドを削除
  - ✅ templates/companies/detail.html から「データソース」表示要素を削除
  - ✅ データベース内のsource_filesフィールドは保持（削除しない）
  - ✅ バックエンドのデータ管理・追跡目的のためデータベースには保持
  - ✅ テスト実装と検証完了 (tests/test_ui_display_constraints.py)
  - _Requirements: UI表示制約_

- [ ] 11. エンドツーエンドテストの実装
- [x] 11.1 フルアクセスユーザーのレビュー閲覧フロー
  - ✅ レビュー詳細ページでpreview_mode条件分岐を確認
  - ✅ コメント表示ロジック（category-comment）の存在確認
  - ✅ 6つの評価項目キー（recommendation, foreign_support, etc.）の存在確認
  - ✅ category_labelsディクショナリの使用確認
  - ✅ 企業詳細ページへの戻るリンク（company_id変数使用）確認
  - ✅ 企業詳細ページのレビュー詳細リンク（review['id']使用）確認
  - ✅ レビューハンドラーのアクセス制御ロジック確認
  - ✅ テスト実装と検証完了 (tests/test_e2e_review_detail_flows.py)
  - _Requirements: 1.3, 1.5, 1.8, 6.1, 6.5_

- [x] 11.2 プレビューユーザーの質問別レビュー閲覧フロー
  - ✅ 企業詳細ページの質問別レビュー一覧リンク確認
  - ✅ category_review_list.htmlテンプレートの存在確認
  - ✅ プレビューモード時のコメントマスキング確認
  - ✅ プレビューユーザー向けCTA表示確認
  - ✅ CategoryReviewListHandler（専用ファイル）の存在確認
  - ✅ カテゴリ別レビュー一覧ハンドラーのアクセス制御ロジック確認
  - ✅ ルーティング定義（/companies/.../reviews/by-category/...）確認
  - ✅ カテゴリ名（category_label）表示確認
  - ✅ 平均評価スコア（category-average）表示確認
  - ✅ レビュー総数（review-count）表示確認
  - ✅ テスト実装と検証完了 (tests/test_e2e_review_detail_flows.py)
...
- [x] 12.1 ページ構成と表示項目のドキュメント作成
  - ✅ 個別レビュー詳細ページの構成を入れ子構造で記載
  - ✅ 質問別レビュー一覧ページの構成を入れ子構造で記載
  - ✅ 企業詳細ページへの統合要素を記載
  - ✅ レビュー一覧ページへの統合要素を記載
  - ✅ requirements.md と design.md に記載
  - _Requirements: ドキュメンテーション要件_
